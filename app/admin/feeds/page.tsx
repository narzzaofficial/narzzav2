...
export default function FeedListPage() {
  ...
  async function loadFeeds() {
    try {
      ...
      // Handle jika data adalah object dengan property feeds, atau langsung array
      const feedsArray = Array.isArray(data) ? data : data.feeds || [];

      // Sort by newest first - dengan explicit type
      setFeeds(feedsArray.sort((a: Feed, b: Feed) => b.id - a.id));
    } catch (error) {
      flash("❌ Gagal memuat data feeds");
      console.error(error);
      setFeeds([]); // Set empty array jika error
    } finally {
      setLoading(false);
    }
  }
  ...
  function getCategoryColor(category: string) {
    switch (category) {
      case "Berita":
        return "bg-sky-100 text-sky-700 border-sky-300 dark:bg-sky-900/30 dark:text-sky-300 dark:border-sky-700";
      case "Tutorial":
      ...
      case "Riset":
        return "bg-fuchsia-100 text-fuchsia-700 border-fuchsia-300 dark:bg-fuchsia-900/30 dark:text-fuchsia-300 dark:border-fuchsia-700";
      default:
        ...
        <div className="glass-panel mb-6">
          ...
          <div className="mt-4 flex items-center gap-4 border-t border-slate-200 pt-4 text-sm dark:border-slate-700">
            <div className="text-slate-600 dark:text-slate-400">
              Menampilkan{" "}
              <span className="font-semibold">{filteredFeeds.length}</span> dari{" "}
                  ...
                  <div className="mb-3 flex items-start justify-between">
                    <div className="flex-1">
                      <div className="mb-1 flex items-center gap-2">
                        ...
                        <span
                          className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold ${getCategoryColor(
                            feed.category
                          )}`}
                        >
                          {feed.category}
